---
description: Regenerate the engagement dashboard and report where things stand and what to run next
---

The dashboard carries the full picture; this command regenerates it,
publishes it, and hands the user the one next move.

1. Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/gen-dashboard.py <engagement-root>`
   (the current directory). It seeds `dashboard.html` at the engagement root
...
3. Report in chat, briefly: current phase and step count, the one next
   ...
   feedback reports).
   Teach the method only on request or on a brand-new engagement.
4. If the directory has no `engagement.md`, this is a brand-new engagement:
